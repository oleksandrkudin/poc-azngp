...
function get_changed_files () {
  ...
  parent_commit=$(git log --pretty=%P -n 1 $(git log -1 --format='%H'))
  changed_files=$(git diff-tree --no-commit-id --name-only ${parent_commit:-"--root"} -r $(git log -1 --format='%H') ${git_base_ref})
